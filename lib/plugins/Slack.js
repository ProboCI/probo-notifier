...
class Slack {
  ...
  constructor(notification, config) {
    this.enabled = typeof notification === 'boolean' ? notification : false;
  ...
  async _process(build) {
    ...
    const message = {
      text: `New Probo build for project \`${build.project.name}\` on branch \`${build.branch.name}\` at ${build.links.build}`
    };
