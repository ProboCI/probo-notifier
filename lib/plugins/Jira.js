...
class Jira extends PluginWithCredentials {
  ...
  send(data) {
    // Only if Jira notifications are enabled and the received event is for
  ...
  /**
   * Sends information on the created build as a Jira comment on the related
   * issue.
   *
  ...
  /**
   * Gets the Jira issue identificator from the branch name.
   *
   * This expects that the branch name has the format
   * ProjectKey-Ticket#-Anything-Else (e.g. XXX-000/yyyyyy-yyyy, where XXX-000
   * is the Jira issue ID).
   *
   * @param {string} branchName - The branch name the build is associated with.
