...
class Server extends EventEmitter {
  ...
  start(cb) {
    this.consumer.start()
      ...
      .catch(err => {
        this.logger.fatal({err: err}, 'Consumer failed to start');
        process.exit(1);
      });
  }
  ...
  messageProcessor(data, cb) {
    this.logger.info('Eventbus event received', {buildId: data.build.id, event: data.event});
    if (data.build.config.notifications) {
      ...
      for (let pluginName in data.build.config.notifications) {
        if (this.notifyPlugins[pluginName]) {
          ...
          for (let notification of notifications) {
            try {
              let plugin = new this.notifyPlugins[pluginName](notification, this.config);
      ...
      this.logger.error('Build does not have any notifications enabled');
      this.emit('buildReceived', data.build);
    }
